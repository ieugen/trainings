...
//        String connectionUrl = "jdbc:h2:~/training-test-jdbc.db";
        ...
        try (Connection conn = DriverManager.getConnection(connectionUrl, "sa", "")) {
            ...
            statement.execute("create table t1(id int primary key auto_increment, name char)");
            statement.execute("insert into t1(name) values ('jdbc')");
            statement.execute("insert into t1(name) values ('java')");
            statement.execute("insert into t1(name) values ('trainig')");
            statement.execute("insert into t1(name) values ('persistence')");
